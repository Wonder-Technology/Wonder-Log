...
let _ =
  describe("Log", () => {
    ...
    test("test", () => {
      Wonder_Console.makeObjInToWindow();
      /* Log.debug(Log.buildDebugMessage(~title="aaa", ~reason="re", ~solution="so", ~description="de", ~params={j|v1:1, v2:2|j}), true); */
      Log.debug(
        Log.buildDebugMessage(~description="de", ~params={j|v1:1, v2:2|j}),
        true,
      );
      let arr = [|1, 2|];
      let obj = {a: 1, b: "bbb"};
      Log.debugJson(
        Log.buildDebugJsonMessage(~description="de", ~var=obj),
        true,
      );
    ...
    test("aaa", () => {
      ...
      Wonder_Console.info("fckkkkk");

      Wonder_Console.trace("fckkkkk");
